...
class HierarchicalRobustAutoRegressiveObservationsNoInputs(_Observations):
    ...
    @property
    def params(self):
        return self.shared_As, self.shared_bs, \
               self.As, self.bs, self.inv_sigmas, self.inv_nus
        
    @params.setter
    def params(self, value):
        self.shared_As, self.shared_bs, \
        self.As, self.bs, self.inv_sigmas, self.inv_nus = value
    ...
    def initialize(self, datas, inputs=None, masks=None, tags=None):
        ...
        for k in range(self.K):
            ts = npr.choice(T-self.lags, replace=False, size=(T-self.lags)//self.K)
            x = np.column_stack([data[ts + l] for l in range(self.lags)])
            y = data[ts+self.lags]
            try: 
                lr = LinearRegression().fit(x, y)
                self.shared_As[k] = lr.coef_[:, :self.D * self.lags]
    ...
    def _compute_mus(self, data, input, mask, tag):
        T, D = data.shape
        ind = self.tags_to_indices[tag]
        As, bs = self.As[ind], self.bs[ind]

        # Instantaneous inputs
        mus = np.zeros((T-self.lags, self.K, D))
    ...
    def _m_step_ar(self, g, expectations, datas, inputs, masks, tags):
        ...
        taus = []
        for x, y in zip(xs, ys):
            Afull = np.concatenate((self.As[g], self.bs[g, :, :, None]), axis=2)
            mus = np.matmul(Afull[None, :, :, :], x[:, None, :, None])[:, :, :, 0]
            sigmas = np.exp(self.inv_sigmas[g])
            nus = np.exp(self.inv_nus[g, :, None])
        ...
        # Fit the weighted linear regressions for each K and D
        J = 1 / eta * np.tile((np.eye(D * lags + 1))[None, None, :, :], (K, D, 1, 1))
        h = 1 / eta * np.concatenate((self.shared_As, self.shared_bs[:, :, None]), axis=2)
        for x, y, Ez, tau in zip(xs, ys, Ezs, taus):
            robust_ar_statistics(Ez, tau, x, y, J, h)
