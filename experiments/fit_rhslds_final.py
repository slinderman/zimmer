...
from zimmer.models import HierarchicalWeakLimitStickyHDPHMMSLDS, HierarchicalRecurrentSLDS
...
# IO
run_num = 1
results_dir = os.path.join("results", "08_16_17", "run{:03d}".format(run_num))
...
def fit_hslds(z_inits, x_inits, ys, masks):
    ...
    for worm, (y, mask) in enumerate(zip(ys, masks)):
        ...
        slds.add_data(data=y, mask=mask, group=worm, inputs=inputs)

        # Initialize continuous latent states
        if x_inits is not None:
            slds.states_list[-1].gaussian_states = x_inits[worm][:, :D_latent]

        # Initialize discrete latent states
        if z_inits is not None:
            slds.states_list[-1].stateseq = z_inits[worm]
        else:
            runlen = 10
    ...
    perm_dynamics_distns = [slds.dynamics_distns[i] for i in np.argsort(newlabels)]
    ...
    z_finals = [relabel_by_permutation(s.stateseq, newlabels) for s in slds.states_list]
...
@cached("hrslds")
def fit_hrslds(z_inits, x_inits, ys, masks):
    ...
    smpls = [update(rslds) for itr in progprint_xrange(N_samples)]
    ...
    perm_z_smpls, newlabels = relabel_by_usage(z_smpls, return_mapping=True)
    perm_dynamics_distns = [rslds.dynamics_distns[i] for i in np.argsort(newlabels)]
    ...
    z_finals = [relabel_by_permutation(s.stateseq, newlabels) for s in rslds.states_list]
    x_finals = [s.smoothed_mus for s in rslds.states_list]
    sigma_x_finals = [s.smoothed_sigmas for s in rslds.states_list]

    return rslds, lls, perm_z_smpls, perm_dynamics_distns, z_finals, x_finals, sigma_x_finals
...
def plot_discrete_state_samples(z_smpls, z_true, filename=None):
...
def plot_changepoint_prs(z_smpls, z_true, title=None, plt_slice=(0, 3239), filename=None):
    # Plot the true and inferred state sequences
...
def plot_3d_dynamics(dynamics_distns, z, x, filename=None):
    for k in range(Kmax):
        ...
        if filename is not None:
            fig.savefig(os.path.join(results_dir, "{}_{}.pdf".format(filename, k)))
            plt.close(fig)
...
if __name__ == "__main__":
    ...
    hslds_z_finals, hslds_x_finals, hslds_sigma_x_finals = \
        fit_hslds(z_inits=None, x_inits=x_inits, ys=datasets, masks=masks)
    ...
    hrslds_z_finals, hrslds_x_finals, hrslds_sigma_x_finals = \
        fit_hrslds(z_inits=hslds_z_finals, x_inits=hslds_x_finals, ys=datasets, masks=masks)

    ### Plot SLDS Results
    # for worm in range(N_worms):
    #     plot_discrete_state_samples(
    #         z_smpls[worm], z_trues[worm],
    #         filename="discrete_state_samples_worm{}.pdf".format(worm+1))
    #     plot_changepoint_prs(
    #         z_smpls[worm], z_trues[worm],
    #         title="Worm {} Discrete States".format(worm+1),
    #         filename="discrete_state_cps_worm{}.pdf".format(worm+1))
    ...
    #                          filename="smoothed_states_worm{}.mp4".format(worm+1))
    #
    # plot_3d_dynamics(
    #     dynamics_distns, np.concatenate(z_finals), np.vstack(x_finals),
    #     filename="dynamics")
