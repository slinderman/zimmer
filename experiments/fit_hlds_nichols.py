...
def _fit_lds(D_latent, alpha_0=3.0, beta_0=2.0,
    ...
    if use_covariates:
        ...
        D_in = 1
        train_inputs = [np.ones((d.shape[0], 1)) for d in datas]
        test_inputs = [np.ones((d.shape[0], 1)) for d in ytests]
...
def plot_likelihoods2(D_latents, llss, hlls, best_index,
                     color, name, axs=None):
    ...
    if do_plot_x_3d:
        for i in range(N_plot):
            plot_3d_continuous_states(xtrains[i], z_true_trains[i], colors,
                                      figsize=(4, 4),
                                      title="{} latent $x$ (Zimmer Lables)".format(worm_names[i]),
                                      results_dir=fig_dir,
                                      filename="xtr_{}.pdf".format(worm_names[i]),
                                      lim=6,
                                      lw=1)
            plt.close("all")
...
def fit_all_models(D_latents=np.arange(2, 21, 2)):
    ...
    for index, (is_hierarchical, use_covariates) in enumerate([(True, False)]):
        ...
        for D_latent in D_latents:
            name = "{}_{}".format(
                "hlds_inputs" if (is_hierarchical and use_covariates)
                else "hlds" if is_hierarchical
                else "lds",
                D_latent
            )
        ...
        # axs = plot_likelihoods2(D_latents, llss, hlls, best_index,
        #                        name=name, color=colors[index], axs=axs)
...
def heldout_neuron_identification(N_heldout=10, D_latent=10, is_hierarchical=True, seed=0):
    ...
    all_ys, all_ms, all_us, _, _, all_neuron_names = load_data(include_unnamed=True)
    ...
    for worm in range(N_worms):
        ...
        others = np.where([name.startswith(worm_names[worm] + "_") for name in all_neuron_names])[0]
...
if __name__ == "__main__":
    ...
    utrains, utests, _ = list(zip(*[_split_test_train(u, train=train) for u, train in zip(us, train_inds)]))
